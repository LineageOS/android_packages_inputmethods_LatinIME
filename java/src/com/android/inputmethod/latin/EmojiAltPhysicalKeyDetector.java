...
final class EmojiAltPhysicalKeyDetector {
    ...
    private abstract class EmojiHotKeys {
        ...
        public void onKeyUp(@Nonnull final KeyEvent keyEvent) {
            ...
            if (mKeySet.contains(key)) {
                if (mCanFire) {
                    if (!keyEvent.isCanceled()|| keyCode == KeyEvent.KEYCODE_ALT_RIGHT || keyCode == KeyEvent.KEYCODE_ALT_LEFT 
                        if (DEBUG) {
                            Log.d(TAG, "EmojiHotKeys.onKeyUp() - " + mName + " - firing action"+" keyCode " +keyCode);
                        }
                        action();
                    } else {
